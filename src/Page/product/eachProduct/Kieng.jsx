  ...
  const fetchProductAll = async (pageNumber = 1, pageSize = 10) => {
    try {
      const response = await api.get(
        `/Product/filter-product?PageNumber=${pageNumber}&PageSize=${pageSize}&CategoryID=${6}`
      );
      ...
      <div className="pb-28 flex justify-evenly relative">
        ...
        >
          {products.map((kg) => (
            ...
            >
              <img src={kg.imageURL} className="w-50 h-50 " alt={kg.name} />
              <h3 className="absolute -bottom-5 font-medium w-50">{kg.name}</h3>
